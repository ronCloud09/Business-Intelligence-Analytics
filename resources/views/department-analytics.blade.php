...
function buildStats(data) {
    ...
    const mappings = {
        'total_revenue': { icon: 'dollar-sign', label: 'Total Revenue', format: v => '₱' + Number(v).toLocaleString() },
        'total_orders': { icon: 'shopping-cart', label: 'Orders', format: v => Number(v).toLocaleString() },
        'new_customers': { icon: 'users', label: 'New Customers', format: v => v },
...
function buildBottomCards(data) {
    ...
    if (data.top_products && data.top_products.length > 0) {
        cards.push({
            ...
            rows: data.top_products.map(p => [p.product_name, p.units_sold, '₱' + Number(p.revenue).toLocaleString()])
        });
    }
...
const deptDescs = {
    ecommerce: 'Online sales performance, orders, and customer acquisition.',
